...
path.src.html[1] = '!' + path.src.html[0].slice(0, -7) + '_*.html';
 ...
 * Video path
 */
path.src.video[0] = path.src.srcPath + path.src.video[0];
...
function ttf2woff2Converter () {
  return gulp.src(path.src.font[0].slice(0, -7) + 'src/*.ttf')
             .pipe(ttf2woff2())
             .pipe(gulp.dest(path.src.font[0].slice(0, -7)));
}
 ...
 */
function otf2ttf () {
  return gulp.src(path.src.font[0].slice(0, -7) + 'src/*')
             .pipe(fonter({
               formats: ['ttf']
             }))
             .pipe(gulp.dest(path.src.font[0].slice(0, -7) + 'src'));
}
...
exports.default = gulp.series(
    gulp.parallel(clean),
    gulp.parallel(njk, scss, script, image, font, video),
